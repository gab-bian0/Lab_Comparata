...
for proteome in *.faa; do
	awk '{if(NR==1) {print $0} else {if($0 ~ /^>/) {print "\n"$0} else {printf $0}}}' "$proteome" > ../${proteome/.faa}"_ol.faa"
done
cd ..
...
for proteome in *.faa; do
	species=$(basename -s _ol.faa "$proteome")
...
for pseudo_file in 00_pseudogene_name/*_pseudogenes_name.txt; do
	...
	while IFS=$'\t' read -r header; do
		sed -E -i "/${header}/{N;d;}" "$species"_ol.faa # N option loads the next line found after the pattern and put it into pattern space too; d delete the pattern space
	done < "$pseudo_file" 
done
